...
async function getDb() {
  ...
  return {
    db: drizzle({ client: pool, schema: { carols }, mode: "default" }),
    pool,
  };
}
...
async function inspect() {
  ...
  try {
    ...
    const emptyLyrics = allCarols.filter(
      (c) => !c.lyrics || c.lyrics.length === 0,
    );
    console.log(`- Missing Lyrics: ${emptyLyrics.length}`);

    const htmlEntities = allCarols.filter((c) =>
      JSON.stringify(c.lyrics).includes("&"),
    );
    console.log(`- Lyrics with HTML entities: ${htmlEntities.length}`);

    const shortLyrics = allCarols.filter(
      (c) => c.lyrics && c.lyrics.length > 0 && c.lyrics.length < 5,
    );
  ...
  try {
    ...
    for (const carol of targetCarols) {
      ...
      try {
        ...
        await db
          .update(carols)
          .set({ title: data.title, lyrics: data.lyrics })
          .where(eq(carols.id, carol.id));
