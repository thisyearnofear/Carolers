...
async function testModel(modelName: string, apiKey: string): Promise<boolean> {
  ...
  try {
    ...
    const prompt = 'What is the title of the Christmas carol that starts with "Silent night"? Reply with just the title.';
    ...
    if (modelName.includes('3')) {
      ...
      const toolResult = await modelWithTools.generateContent('Find me carols about snow');
      const toolResponse = await toolResult.response;
      const functionCalls = toolResponse.functionCalls();

      if (functionCalls && functionCalls.length > 0) {
        console.log(`✅ Function calling works! Called: ${functionCalls[0].name}`);
        console.log(`   Args: ${JSON.stringify(functionCalls[0].args)}`);
      } else {
        console.log(`ℹ️  No function calls generated (model may have answered directly)`);
      }
    }
...
async function testAllModels() {
  ...
  for (const [model, success] of Object.entries(results)) {
    console.log(`${success ? '✅' : '❌'} ${model}: ${success ? 'Working' : 'Failed'}`);
  ...
  if (successCount === 0) {
    ...
    console.log('2. Check if you have access to these models in Google AI Studio');
    ...
    process.exit(1);
  } else if (results['gemini-3-flash-preview'] || results['gemini-3-pro-preview']) {
