...
export async function POST(req: Request) {
  try {
    const { title, artist, analysisType } =
      (await req.json()) as DeepAnalysisRequest;
...
Help a director understand how to arrange this for different ensemble sizes.`,
    ...
    const systemPrompts: Record<string, string> = {
      composition:
        "You are an expert music theorist and composer. Analyze carols with deep musical knowledge.",
      performance:
        "You are an experienced vocal coach and choir director. Provide practical performance guidance.",
      cultural:
        "You are a musicologist and cultural historian specializing in Christmas traditions.",
      harmony:
        "You are an accomplished choral arranger with expertise in voice leading and ensemble singing.",
    };

    const prompt = prompts[analysisType] || prompts.composition;
    const systemPrompt =
      systemPrompts[analysisType] || systemPrompts.composition;
